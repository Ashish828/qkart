  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChangeCapture={(e)=> handleNewAddress( prev => ({...prev, value: e.target.value}))}
      />
      <Stack direction="row" my="1rem">
        ...
        <Button
        onClick={() => handleNewAddress((currNewAddress) => ({
          ...currNewAddress,
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const res = await axios.post(config.endpoint + "/user/addresses", {address: newAddress}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  const validateRequest = (items, addresses) => {
    if(parseInt(localStorage.getItem("balance")) < getTotalCartValue(items)){
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      localStorage.setItem("balance", parseInt(localStorage.getItem("balance") - getTotalCartValue(items)))
      history.push("/thanks");
  ...
  useEffect(() => {
    const onLoadHandler = async () => {
      ...
      setAddresses({ ...addresses, all: await getAddresses(token) });
    ...
    <>
      ...
      <Grid container>
              ...
              
              {
                addresses.all.length > 0 && (
                  addresses.all.map( add =>
                    <Box 
                     key={add._id} 
                     className={`address-item ${addresses.selected === add._id ? "selected" : "not-selected"}`} 
                     sx={{display: "flex", justifyContent:"space-between", px:2, mb:1}} 
                     onClick={() => setAddresses({all: addresses.all, selected: add._id})}
                     >
                     <Typography my="1rem">
                     {add.address}
                    </Typography>
        ...
        <Grid item xs={12} md={3} bgcolor="#E9F5E1">
          ...
          <Box className="cart" sx={{px: 2, py:4}}>
            <Typography 
            color="#3C3C3C"
            fontWeight="700"
            fontSize="2rem"
